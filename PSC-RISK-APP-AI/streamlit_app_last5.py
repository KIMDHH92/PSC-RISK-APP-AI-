...
# 경로 설정 (회사/집 자동 스위칭)
# =========================
COMPANY_BASE = r"C:\Users\user\OneDrive - 한국선급\바탕 화면\AI 공모\2. PSC 수검 선별"
HOME_BASE    = r"C:\Users\pc\OneDrive\바탕 화면\2. PSC 수검 선별"

def choose_base_path():
    if os.path.exists(COMPANY_BASE):
        return COMPANY_BASE
...
DB_FILE = os.path.join(BASE_PATH, "0.전체선박", "20250806_3177척.xlsx")
ETA_ROOT = os.path.join(BASE_PATH, "1.ETA_ALL")
DETENTION_FOLDER = os.path.join(BASE_PATH, "2.출항정지이력")
DEFICIENCY_FOLDER = os.path.join(BASE_PATH, "3. PSC Deficiency Item List")
CHECKLIST_FOLDER = os.path.join(BASE_PATH, "4. 지적사항별 Checklist")

# NEW: MOU INFO (Selection Scheme + Last Periodical Survey)
MOU_INFO_FOLDER = os.path.join(BASE_PATH, "5. MOU INFO", "MoU Information&Last Periodical Survey")
MOU_INFO_MNT = "/mnt/data/MoU Information&Last Periodical Survey.csv"

# 신규 업로드 파일 우선 사용(있으면)
CHECKLIST_PATH_MNT = "/mnt/data/지적사항별 Checklist 정리.xlsx"
CHECKLIST_OVERRIDE = CHECKLIST_PATH_MNT if os.path.exists(CHECKLIST_PATH_MNT) else None

PORT_COORDS_LOCAL  = os.path.join(BASE_PATH, "port_coords.csv")
PORT_COORDS_MNT    = "/mnt/data/port_coords.csv"
...
def load_detention_data(folder):
    path = get_latest_file(folder, exts=(".xlsx",))
    if not path:
        return pd.DataFrame()
    df = pd.read_excel(path)
    rename_map = {
...
def load_deficiency_data(folder):
    path = get_latest_file(folder, exts=(".xlsx",".csv"))
    if not path:
        return pd.DataFrame()
...
def _read_flag_xlsx_map(xlsx_path="flag.xlsx"):
    try:
    ...
    for c in ["MOU","FLAG","LEVEL"]:
        long_df[c] = long_df[c].astype(str).strip()
    import re as _re
    def _split_flags(s):
    ...
    try:
        with st.expander("🧠 AI 자동 학습/예측 로그(접힘)", expanded=False):
            ...
            if td is None or td.empty:
                st.caption("⚠️ 학습 데이터셋 생성 실패/비어있음 → 룰 100% 폴백")
            else:
                target_imos = []
                try:
        ...
        if ship_db.empty or "imo no." not in ship_db.columns or "ship name" not in ship_db.columns:
            ...
            if res.empty:
                ...
                if row is None:
                    ...
                    # ETA 최신값
                    eta_rows = eta_df_range[eta_df_range['vessel_imo'] == imo].copy()
